...
public class BannerViewPager extends LinearLayout implements ViewPager.OnPageChangeListener {
    ...
    private int mIndicatorDefaultImage = R.mipmap.icon_ban_nor;
    private int mIndicatorCurrentImage = R.mipmap.icon_ban_focus;
    private Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            if (msg.what == LOOP && imageViews.size() != 0) {
                ...
                handler.postDelayed(runnable, bannerGetTime.getBannerTime());
                return;
            }
     ...
     * G构建显示图片view
     *
     * @param imageList 需要显示张数
     */
    private void creatImageView(List<String> imageList) {
        // List<ImageView> views = new ArrayList<ImageView>(); // 需要添加Banne 图片

        // 将最后一个ImageView添加进来
        imageViews.add(ViewFactory.getImageView(mContext, imageList.get(imageList.size() - 1)));
        for (int i = 0; i < imageList.size(); i++) {
            imageViews.add(ViewFactory.getImageView(mContext, imageList.get(i)));
        }
        // 将第一个ImageView添加进来
        imageViews.add(ViewFactory.getImageView(mContext, imageList.get(0)));
     ...
     */
    public void setData( List<String> list, int showPosition, OnItemClickListener listener) {
    ...
    private void setIndicator(int selectedPosition) {
        ...
        if (onBannerScrollListener != null) {
            onBannerScrollListener.onCurrentClick(selectedPosition);
        }

        for (int i = 0; i < indicators.length; i++) {
            indicators[i]
                    .setBackgroundResource(mIndicatorDefaultImage);
            indicators[i].setPadding(5, 5, 5, 5);
        }
        if (indicators.length > selectedPosition)
            indicators[selectedPosition]
                    .setBackgroundResource(mIndicatorCurrentImage);
        indicators[selectedPosition].setPadding(5, 5, 5, 5);
     ...
     * @param imageId
     */
    public void setIndicatorDefaultImage(int imageId) {
        this.mIndicatorDefaultImage = imageId;
    }
     ...
     * @param imageId
     */
    public void setIndicatorCurrentImage(int imageId) {
        this.mIndicatorCurrentImage = imageId;
    }
    ...
    public void setIndicatorImage(int[] imageId) {
        if (imageId != null && imageId.length == 2) {
            setIndicatorDefaultImage(imageId[0]);
            setIndicatorCurrentImage(imageId[1]);
        }
    }
     ...
     * @param listener
     */
    public void setBannerScrollListener(OnBannerScrollListener listener) {
        this.onBannerScrollListener = listener;
    }
    ...
    final Runnable runnable = new Runnable() {
        ...
        public void run() {
            if (getContext() != null
                ...
                if (now - releaseTime > (bannerGetTime.getBannerTime()) - 500) {
    ...
    public interface OnBannerScrollListener {
         ...
         * @param postion 位置
         */
        void onCurrentClick(int postion);
    }
    ...
    private BannerGetTime bannerGetTime;

    public interface BannerGetTime {
        int getBannerTime();


    }

    public void initBannerTime(BannerGetTime bannerGetTimes) {
        this.bannerGetTime = bannerGetTimes;
    }
